...
​
  ...
  ngOnInit() {
    ...
    this.treeEventListener(this.treeData[0].children[0])
  }
  ...
  treeEventListener(event) {
    this.createArray = [];
      let eventData={identifier:event.id,criteriaName:event.title}
      this.criteriaId = eventData?.identifier;
      const data = this.treeService.getFirstChild();
      const hierarchy = this.editorService.getHierarchyObj(data, '', eventData?.identifier);
      this.questionService.getQuestionList(_.get(hierarchy[eventData?.identifier], 'children'), 'editorState')
